...
class SongBloom_Sampler:
    ...
    def __init__(self, compression_model: StableVAE, diffusion: MVSA_DiTAR, lyric_processor_key,
        ...
        self.lyric_processor = key2processor.get(lyric_processor_key) if lyric_processor_key is not None else lambda \
                x: x
    ...
    @classmethod
    def build_from_trainer(cls, cfg, fabric: Fabric, strict=True, compile: bool = True):
        # todo: this should really use empty_weight_init from accelerate
        with fabric.init_module(empty_init=True):
            model_light = SongBloom_PL(cfg)

        incompatible = model_light.load_state_dict(torch.load(cfg.pretrained_path), strict=strict)
        if compile:
            model_light = torch.compile(model_light)
        fabric.setup(model_light, move_to_device=True)
    ...
    def dtype(self) -> torch.dtype:
        return next(self.compression_model.vae.parameters()).dtype
    ...
    def generate(self, lyrics, prompt_wav) -> tp.Union[torch.Tensor, tp.Tuple[torch.Tensor, torch.Tensor]]:
        ...
        attributes, _ = self._prepare_tokens_and_attributes(
            conditions={"lyrics": [self._process_lyric(lyrics)], "prompt_wav": [prompt_wav]},
